...
contract AuctionBase is Pausable {
  ...
  event BidCreated(
    uint256 id, address nftAddress, uint256 tokenId, address bidder, uint256 bid
  );
  event AuctionNFTWithdrawal(
    uint256 id, address nftAddress, uint256 tokenId, address withdrawer
  );
  event AuctionFundWithdrawal(
    uint256 id, address nftAddress, uint256 tokenId, address withdrawer, uint256 amount
  );
